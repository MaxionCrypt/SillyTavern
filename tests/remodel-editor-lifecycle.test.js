// A full editor-mode turn at the lifecycle level: the two-agent Director never
// runs, the Narrator drafts, the Loom reconciles at completeVisibleRun, and the
// turn settles waiting for the user. (State recording by the Loom is covered by
// remodel-director-editor-turn.test.js; this suite covers the shared turn
...
beforeEach(() => {
    ...
    setLiveDirectionTestAdapters({
        // The two-agent Director engine is gone — it must never be reached.
        requestDirection: async () => { throw new Error('The two-agent Director must not run in editor mode'); },
        generatePerformer: speak,
        // No directorEdit adapter: runDirectorEdit falls back to the draft
...
test('an editor turn commits the narrator draft and waits for the user', async () => {
    await requestNextDirection(scene);
    expect(await until(() => getLiveDirectionRun()?.state === 'Waiting for you')).toBe(true);
    // The Narrator spoke (the Director never ran — its adapter would have thrown).
