...
function describeNoGoals(retrieval, mechanicsEnabled) {
    ...
    return `No Goals are open in this Scene.
A Goal is a meaningful unresolved outcome someone is actively trying to achieve. Create one with goal.create only when the fiction establishes an outcome worth tracking, never merely because a named character lacks one.
Every Goal has a meaningful Success Rate: its holder's present chance of achieving it. A Goal earns its place only while someone can still pursue it. Close the ones the fiction has overtaken with goal.edit - impossible when events put it out of reach, abandoned when its holder gives up, achieved when they get it.`;
}

function rateGuidance() {
    return `Every active Goal's Success Rate is the chance its holder still has of achieving it from the position the fiction has reached. A turn that materially helps or obstructs the Goal should change that rate with goal.edit even when no roll occurs. A Goal description guides measurement but is not an exhaustive whitelist; refine an incomplete description with goal.edit instead of using it to dismiss a consequence. These are reference points, not a list to choose from — state the number that fits:
  5 nearly impossible · 15 extreme · 30 difficult · 50 uncertain · 70 favourable · 85 strongly favoured · 95 nearly assured
Rates hold between 5 and 95: a Goal already certain or already lost is a status, not a roll. Ask what materially changed because this turn happened. For scale, a small pressure is a few points, a meaningful turn is nearer seven, and a decisive reversal is twenty or more.
