...
import { renderLoomAction } from './narrator-prompt.js';
...
function uiLocationScrollKey() {
    const state = getSessionState();
    const archive = loomArchive.open ? 'archive' : 'scenes';
...
async function runPromptPreviewDryRun(generationType, { composerText: composerTextOverride, narratorGrounding, narratorRecall, narratorNote, stripNativeNewChatBootstrap = false } = {}) {
    ...
    // The live request resolves Archive state into the recipe's Narrator
    // Grounding macro immediately before native assembly. Preview uses that
    // same content-prompt seam and clears it afterward.
...
function renderTimelineFocus(timeline, store) {
    ...
    return `
        <section class="remodel-timeline-focus remodel-route-overview ${hasImage ? 'has-image' : ''}" style="${rootStyle}" aria-label="Timeline ${escapeAttribute(timeline.title)}">
            ...
            ${loomArchive.open ? `<div class="remodel-route-layout is-archive">${renderLivingLoreArchive(timeline)}</div>` : `
...
async function openStoryToolPanel(tool, trigger = null) {
    ...
    if (tool === 'archive') {
        closeStoryToolPanel();
        openSceneLivingLorePanel(getActiveScene());
        return;
    }
    if (tool === 'loom') {
        closeStoryToolPanel();
...
async function openRoleplayPromptPreview() {
    ...
    try {
        ...
        if (directed) warnings.push('The Loom has not run: it receives the completed Narrator draft, then reconciles continuity and uncertain outcomes before the response becomes visible. Preview never rolls or mutates.');
...
async function handleRoleplaySend(root) {
    ...
    if (isDirectedLiveScene(scene)) {
        ...
        try {
            await directedTurnController.start({ scene, text: value });
        } finally {
...
function renderRoleplayScene() {
    ...
    // loom.action needs the live action and is filled at generation time, so it
    // resolves empty here and is overwritten then.
...
function ensureRoleplayPanelGroup() {
    ...
    group.innerHTML = `
        <button type="button" class="remodel-rp-panel-icon" data-remodel-rp-action="open-lore" title="Living Lore Archive" aria-label="Living Lore Archive">
            <i class="fa-solid fa-box-archive" aria-hidden="true"></i>
        </button>
