...
A roleplay turn is written by **one mind**, not two. That mind is an ordinary SillyTavern **character card** — for the omniscient case, a card we call **"Story Loom"** whose description/personality/system-prompt is *how it reasons and how it writes*. The card generates through **native generation** (its full configured prompt) so it is as capable as any normal chat, with the **archivist** (structured story-state) injected as context. After the prose lands, a cheap **Pass 2 extraction** reads the model's **reasoning** plus the delivered prose and records what changed into the archivist. There is no separate Director agent; there is no notebook. The system deliberately targets **reasoning-capable models only**.
...
### 3.1 Pass 1 — Narrate (the only creative call)

Native SillyTavern generation as the bound narrator card. Full configured prompt (system prompt, card, persona, world info, examples, history), **plus** the archivist state injected into the "Director's Notes" slot as readable markdown, **plus** the always-on append-only directive. The model **reasons first** (thinking channel), then writes the prose. Streams into the existing reveal pipeline; `finalizeRunMessage` writes + saves.

### 3.2 Pass 2 — Extract (mechanical, reasoning-fed)

After the prose is accepted, a cheap non-streaming call reads **the delivered prose + Pass 1's reasoning** and emits a state fence → `executeDirectionRequests` → archivist + mechanics (one atomic transaction). Records narrative state (events, facts, char-states, beat, secrets) **and** variable/goal changes, resolved against the run's address book. **Pass 2 is retained permanently — not folded back into one call** (a state fence inside the thinking channel is too flaky to be the reliable path).

### 3.3 The archivist (SHIPPED)

`archivist-store.js` + archivist capabilities in `mechanics-capabilities.js`. Per timeline+scene: `scene_fact`, `event` (append-only), `char_state`, `beat` (singleton), `secret` (never shown to the narrator). Read as markdown via `buildNarratorArchivistSections` (secrets filtered, fail-closed); written as JSON via the state fence. It is the single memory — **the notebook is removed.**
...
Pass 2 reads the model's **reasoning channel** (the hidden thinking a reasoning model emits before its answer — `streamChatPrompt` already captures it). The narrator card **instructs what that reasoning contains**: e.g. *"before you write, in your private reasoning note any tracked value that changed, any fact established, and who witnessed it."*
...
- **Direction cards** in the stream re-source from the **archivist** (current beat + recent state changes) instead of notebook entries.
...
**Built and green (current HEAD):** archivist store + capabilities; native narrator with full prompt; archivist injection + append-only directive; Pass 2 extraction (narrative + mechanics, reasoning-fed); solo mode as a selectable/default-able mode (`solo-direction.js`, the two seams); `setLiveDirectionMode` + Engine toggle; solo Stop = cut off; solo lifecycle tests.

**Remaining (ordered in the handoff plan):**
1. **Delete the Director** — remove the turn-flow branch (always solo), retire the Director-via-turn tests (cascades to ~5 files — see handoff), delete Director code + notebook. Green after each step, own branch.
2. **Reasoning gate** — empirical warn/block on empty reasoning + soft pre-flight; surface in UI.
3. **Story Loom default card** — ship a tuned omniscient-narrator card.
